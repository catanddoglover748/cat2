...
# === 1. ライブラリ：必要なライブラリのインポート（ファイルの先頭） =======================================================================
...
# === 6.CIK 解決 ティッカー（AAPLなど）からSEC用のCIKコードを探す 終了==================================================================================================

# === 7.facts取得　会社ファクト（XBRL facts）を取得 記号の揺れ（BRK-B/BRK.Bなど）も吸収 ==================================================================================================
@cache_days(30)
def sec_company_facts(cik: str) -> dict:
    ...
    r = sec_get(url)
    return r.json()
# ===  7.facts取得　会社ファクト（XBRL facts）を取得 記号の揺れ（BRK-B/BRK.Bなど）も吸収 終了==================================================================================================

# ===  8.facts探索ユーティリティ　候補キーから fact を抽出（最新の四半期/年次を優先）=======================================================================================
GAAP_REVENUE_KEYS = [
...
# ===  8.facts探索ユーティリティ候補キーから fact を抽出（最新の四半期/年次を優先）終了=======================================================================================
...
# ===27. ⏬ 決算データ　デバッグ用：SECの生データを確認するための関数 =================================================================================================
 # ===PATCH:A ===============================================
